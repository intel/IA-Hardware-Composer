...
int HwfLayer::InitFromHwcLayer(hwf_layer_t *sf_layer)
{
  ...
  hwc_layer_->SetSourceCrop(hwcomposer::HwcRect<float>(
        sf_layer->srcRect.left, sf_layer->srcRect.top,
        sf_layer->srcRect.right, sf_layer->srcRect.bottom));

  hwc_layer_->SetDisplayFrame(hwcomposer::HwcRect<int>(
        sf_layer->destRect.left, sf_layer->destRect.top,
        sf_layer->destRect.right, sf_layer->destRect.bottom),
        0, 0);
  ...
  ITRACE("%.f %.f %.f %.f -> %d %d %d %d blending(%x) alpha(%d)", 
      ...
      sf_layer->destRect.left, sf_layer->destRect.top,
      sf_layer->destRect.right, sf_layer->destRect.bottom,
      sf_layer->blendMode, sf_layer->globalAlpha);
  return 0;
}
...
int HwfDevice::flip(struct hwf_device_t *device,
      int dispCount, hwf_display_t** displays)
{
  CTRACE();
  ITRACE("dispCount: %d\n", dispCount);
  ...
  for (int i = 0; i < dispCount; ++i) {
    ...
    for (size_t j = 0; j < num_dc_layers; ++j) {
      ...
      if (!sf_layer || !sf_layer->target || (sf_layer->flags & HWF_LAYER_IGNORED)) {
        ITRACE("Skip layer: %p %p %x", sf_layer, sf_layer->target, sf_layer->flags);
        ITRACE("(%f %f %f %f => %d %d %d %d)",
              sf_layer->srcRect.left, sf_layer->srcRect.top,
              sf_layer->srcRect.right, sf_layer->srcRect.bottom,
              sf_layer->destRect.left, sf_layer->destRect.top,
              sf_layer->destRect.right, sf_layer->destRect.bottom);
      ...
      if (!native_display->gl_composition_ && (sf_layer->composeMode == HWF_FB_TARGET)) {
        ITRACE("Skip layer: %d %d", native_display->gl_composition_, sf_layer->composeMode);
        continue;
      }
...
int HwfDevice::setEventState(struct hwf_device_t* device, int disp,
...
int HwfDevice::lookup(struct hwf_device_t* device, int what, int* value)
{
  ...
  case HWF_SUITABLE_DISPLAY_TYPES:
    if (value)
      *value = HWF_DISPLAY_PRIMARY_BIT | HWF_DISPLAY_EXTERNAL_BIT | HWF_DISPLAY_VIRTUAL_BIT;
    break;
...
/* Callback function */
void HwfDevice::registerCallback(struct hwf_device_t* device, hwf_callback_t const* callback)
{
  CTRACE();
  ...
  display->RegisterVsyncCallback(std::move(vsync_callback), HWF_DISPLAY_PRIMARY);
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    extended.at(i).display_->RegisterVsyncCallback(std::move(extended_callback), disp_id);

    auto hotplug_callback = std::make_shared<IAHotPlugEventCallback>(callback);
    extended.at(i).display_->RegisterHotPlugCallback(std::move(hotplug_callback), disp_id);
  }
          ...
          uint32_t config, const uint32_t* attributes, int32_t* values)
{
  ...
  for (int i = 0; attributes[i] != HWF_DISPLAY_NO_ATTRIBUTE; ++i) {
    ...
    case HWF_DISPLAY_WIDTH:
      ret = temp->GetDisplayAttribute(config, hwcomposer::HWCDisplayAttribute::kWidth, &values[i]);
      break;
    case HWF_DISPLAY_HEIGHT:
      ret = temp->GetDisplayAttribute(config, hwcomposer::HWCDisplayAttribute::kHeight, &values[i]);
      break;
    case HWF_DISPLAY_VSYNC_PERIOD: // in nanoseconds
      ret = temp->GetDisplayAttribute(config, hwcomposer::HWCDisplayAttribute::kRefreshRate, &values[i]);
      break;
    case HWF_DISPLAY_DPI_X: // Dots per 1000 inches
      ret = temp->GetDisplayAttribute(config, hwcomposer::HWCDisplayAttribute::kDpiX, &values[i]);
      break;
    case HWF_DISPLAY_DPI_Y: // Dots per 1000 inches
      ret = temp->GetDisplayAttribute(config, hwcomposer::HWCDisplayAttribute::kDpiY, &values[i]);
      break;
    default:
      values[i] = -1;
      return -1;
...
int32_t hwf_open(struct hwf_device_t** device, const VendorModule* module)
{
  ...
  //virtual display
  hwf_device->virtual_display_.display_ = p_gpu_device->CreateVirtualDisplay(HWF_DISPLAY_VIRTUAL);
