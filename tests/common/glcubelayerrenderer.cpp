...
public:
  ...
  bool Initialize (struct gbm_device * gbm)
  {
    ...
    image_ = gl_->eglCreateImageKHR (gl_->display, EGL_NO_CONTEXT,
				     EGL_LINUX_DMA_BUF_EXT,
				     nullptr, khr_image_attrs);
...
bool
  GLCubeLayerRenderer::Init (uint32_t width, uint32_t height,
			     uint32_t format, glContext * gl,
			     const char *resource_path)
{
  ...
  if (enable_texture_)
    {
      glBufferData (GL_ARRAY_BUFFER,
		    sizeof (vVertices) + sizeof (vColors) +
		    sizeof (vNormals) + sizeof (vTexCoord), 0,
		    GL_STATIC_DRAW);
    }
  else
    {
  ...
  glBufferSubData (GL_ARRAY_BUFFER, positionsoffset_, sizeof (vVertices),
		   &vVertices[0]);
  glBufferSubData (GL_ARRAY_BUFFER, colorsoffset_, sizeof (vColors),
		   &vColors[0]);
...
void
GLCubeLayerRenderer::UpdateStreamTexture (unsigned long usec)
{
// 100% every 2 sec
...
void
GLCubeLayerRenderer::glDrawFrame ()
{
  if (enable_texture_)
    {
