...
void VideoLayerRenderer::Draw(int64_t* pfence) {
  ...
  for (int i = 0; i < planes_; i++) {
    ...
    uint32_t lineBytes = get_linewidth_from_format(native_handle_.import_data.format, width,  i);
    uint32_t readHeight = 0;
    uint32_t planeHeight = get_height_from_format(native_handle_.import_data.format, height, i);
