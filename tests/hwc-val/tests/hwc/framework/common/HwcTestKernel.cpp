...
std::shared_ptr<DrmShimBuffer> HwcTestKernel::RecordBufferState(
  ...
  std::shared_ptr<HwcTestBufferObject> bo = GetBufferObject(boHandle);
  std::shared_ptr<DrmShimBuffer> boBuf;
...
// Transfer indices from old buffer instance to new one.
  if (existingBuf.get())
  {
    ...
    for (uint32_t i = 0; i < buf->GetBos().size(); ++i) {
      std::shared_ptr<HwcTestBufferObject> existingBo =
          existingBuf->GetBos().itemAt(i);
      std::shared_ptr<HwcTestBufferObject> newBo = existingBo->Dup();
