...
/** \addtogroup HwcTestNotification Notification
    \ingroup UseCaseTests
    @{
        \brief Create surfaces to mimic the Android home screen with a
                notification pop up.
    @}
*/
...
HwcTestTest::HwcTestTest(int argc, char** argv) : HwcTestBase(argc, argv) {
  mTestName = "hwc_notification_test";
}
...
int HwcTestTest::Run(void) {
  ...
  SurfaceSender::SurfaceSenderProperties sSSP5(
      SurfaceSender::epsNotificationPanel);
  CreateSurface(sSSP5);
