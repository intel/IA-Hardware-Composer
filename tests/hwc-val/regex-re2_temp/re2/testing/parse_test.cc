...
static Regexp::ParseFlags kTestFlags = Regexp::MatchNL | Regexp::PerlX |
                                       Regexp::PerlClasses |
...
static Test tests[] = {
    ...
    {"(?i)\\W",
     "cc{0-0x2f 0x3a-0x40 0x5b-0x5e 0x60 0x7b-0x17e 0x180-0x2129 "
     "0x212b-0x10ffff}"},
    ...
    {"[^ ]", "cc{0-0x9 0xb-0x1f 0x21-0x10ffff}",
     Regexp::NeverNL | Regexp::FoldCase},
    ...
    {"[^ \f]", "cc{0-0x9 0xb 0xd-0x1f 0x21-0x10ffff}",
     Regexp::NeverNL | Regexp::FoldCase},
    ...
    {"[^ \r]", "cc{0-0x9 0xb-0xc 0xe-0x1f 0x21-0x10ffff}",
     Regexp::NeverNL | Regexp::FoldCase},
    ...
    {"[^ \v]", "cc{0-0x9 0xc-0x1f 0x21-0x10ffff}",
     Regexp::NeverNL | Regexp::FoldCase},
    ...
    {"\\S", "cc{0-0x8 0xb 0xe-0x1f 0x21-0x10ffff}", Regexp::PerlClasses},
    {"\\S", "cc{0-0x8 0xb 0xe-0x1f 0x21-0x10ffff}",
     Regexp::PerlClasses | Regexp::FoldCase},
    {"\\S", "cc{0-0x8 0xb 0xe-0x1f 0x21-0x10ffff}",
     Regexp::PerlClasses | Regexp::NeverNL},
...
void TestParse(const Test* tests, int ntests, Regexp::ParseFlags flags,
  ...
  for (int i = 0; i < ntests; i++) {
    ...
    CHECK(re[i] != NULL) << " " << tests[i].regexp << " " << status.Text();
    string s = re[i]->Dump();
...
TEST(TestParse, FoldCase) {
  TestParse(foldcase_tests, arraysize(foldcase_tests), Regexp::FoldCase,
            "foldcase");
}
...
TEST(TestParse, Literal) {
  TestParse(literal_tests, arraysize(literal_tests), Regexp::Literal,
            "literal");
}
...
TEST(TestParse, MatchNL) {
  TestParse(matchnl_tests, arraysize(matchnl_tests), Regexp::MatchNL,
            "with MatchNL");
}
...
TEST(TestParse, NoMatchNL) {
  TestParse(nomatchnl_tests, arraysize(nomatchnl_tests), Regexp::NoParseFlags,
            "without MatchNL");
}
...
const char* badtests[] = {
    ...
    "([a-z)",     "x{1001}",
    "\xff",  // Invalid UTF-8
    "[\xff]",     "[\\\xff]", "\\\xff", "(?P<name>a", "(?P<name>", "(?P<name",
    "(?P<x y>a)", "(?P<>a)",  "[a-Z]",  "(?i)[a-Z]",  "a{100000}", "a{100000,}",
};
