...
void MemoryUsage() {
  ...
  fprintf(stderr, "sizeof: PCRE=%d RE2=%d Prog=%d Inst=%d\n",
          static_cast<int>(sizeof(PCRE)), static_cast<int>(sizeof(RE2)),
          static_cast<int>(sizeof(Prog)), static_cast<int>(sizeof(Prog::Inst)));
}
...
void Search_Success1_BitState(int i, int n) {
  SearchSuccess(i, n, ".*.$", SearchBitState);
}
...
void Search_Success1_Cached_DFA(int i, int n) {
  SearchSuccess(i, n, ".*.$", SearchCachedDFA);
}
void Search_Success1_Cached_PCRE(int i, int n) {
  SearchSuccess(i, n, ".*.$", SearchCachedPCRE);
}
void Search_Success1_Cached_RE2(int i, int n) {
  SearchSuccess(i, n, ".*.$", SearchCachedRE2);
}

BENCHMARK_RANGE(Search_Success1_Cached_DFA, 8, 16 << 20)
    ->ThreadRange(1, NumCPUs());
#ifdef USEPCRE
BENCHMARK_RANGE(Search_Success1_Cached_PCRE, 8, 16 << 20)
    ->ThreadRange(1, NumCPUs());
#endif
BENCHMARK_RANGE(Search_Success1_Cached_RE2, 8, 16 << 20)
    ->ThreadRange(1, NumCPUs());
...
void SearchDigits(int iters, SearchImpl* search) {
  ...
  int len = strlen(text);
  BenchmarkMemoryUsage();
  search(iters, "([0-9]+)-([0-9]+)-([0-9]+)", StringPiece(text, len),
         Prog::kAnchored, true);
...
void Parse_CachedDigits_OnePass(int i) {
  Parse3Digits(i, Parse3CachedOnePass);
}
void Parse_CachedDigits_PCRE(int i) {
  Parse3Digits(i, Parse3CachedPCRE);
}
void Parse_CachedDigits_RE2(int i) {
  Parse3Digits(i, Parse3CachedRE2);
}
...
void Parse_DigitDs_NFA(int i) {
  Parse3DigitDs(i, Parse3NFA);
}
void Parse_DigitDs_OnePass(int i) {
  Parse3DigitDs(i, Parse3OnePass);
}
void Parse_DigitDs_PCRE(int i) {
  Parse3DigitDs(i, Parse3PCRE);
}
...
void Parse_Split_NFA(int i) {
  Parse1Split(i, Parse1NFA);
}
void Parse_Split_OnePass(int i) {
  Parse1Split(i, Parse1OnePass);
}
void Parse_Split_PCRE(int i) {
  Parse1Split(i, Parse1PCRE);
}
void Parse_Split_RE2(int i) {
  Parse1Split(i, Parse1RE2);
}
void Parse_Split_BitState(int i) {
  Parse1Split(i, Parse1BitState);
}
...
void Parse_CachedSplit_NFA(int i) {
  Parse1Split(i, Parse1CachedNFA);
}
void Parse_CachedSplit_OnePass(int i) {
  Parse1Split(i, Parse1CachedOnePass);
}
void Parse_CachedSplit_PCRE(int i) {
  Parse1Split(i, Parse1CachedPCRE);
}
void Parse_CachedSplit_RE2(int i) {
  Parse1Split(i, Parse1CachedRE2);
}
void Parse_CachedSplit_BitState(int i) {
  Parse1Split(i, Parse1CachedBitState);
}
...
void Parse_SplitHard_NFA(int i) {
  Parse1SplitHard(i, Parse1NFA);
}
void Parse_SplitHard_PCRE(int i) {
  Parse1SplitHard(i, Parse1PCRE);
}
void Parse_SplitHard_RE2(int i) {
  Parse1SplitHard(i, Parse1RE2);
}
void Parse_SplitHard_BitState(int i) {
  Parse1SplitHard(i, Parse1BitState);
}
...
void Parse_CachedSplitHard_NFA(int i) {
  Parse1SplitHard(i, Parse1CachedNFA);
}
void Parse_CachedSplitHard_PCRE(int i) {
  Parse1SplitHard(i, Parse1CachedPCRE);
}
void Parse_CachedSplitHard_RE2(int i) {
  Parse1SplitHard(i, Parse1CachedRE2);
}
...
void RunBuild(int iters, const string& regexp,
              void (*run)(int, const string&)) {
  run(iters, regexp);
  SetBenchmarkItemsProcessed(iters);
}
...
void BM_PCRE_Compile(int i) {
  RunBuild(i, FLAGS_compile_regexp, CompilePCRE);
}
void BM_Regexp_Parse(int i) {
  RunBuild(i, FLAGS_compile_regexp, ParseRegexp);
}
...
void BM_RE2_Compile(int i) {
  RunBuild(i, FLAGS_compile_regexp, CompileRE2);
}
...
void SearchDFA(int iters, const char* regexp, const StringPiece& text,
  ...
  for (int i = 0; i < iters; i++) {
    ...
    CHECK(prog);
    bool failed = false;
    CHECK_EQ(prog->SearchDFA(text, NULL, anchor, Prog::kFirstMatch, NULL,
                             &failed, NULL),
             expect_match);
    CHECK(!failed);
    delete prog;
...
void Parse3NFA(int iters, const char* regexp, const StringPiece& text) {
  for (int i = 0; i < iters; i++) {
    ...
    CHECK(
        prog->SearchNFA(text, NULL, Prog::kAnchored, Prog::kFullMatch, sp, 4));
    delete prog;
    re->Decref();
  }
}
...
void Parse1NFA(int iters, const char* regexp, const StringPiece& text) {
  for (int i = 0; i < iters; i++) {
    ...
    CHECK(
        prog->SearchNFA(text, NULL, Prog::kAnchored, Prog::kFullMatch, sp, 2));
    delete prog;
    re->Decref();
  }
}
...
static string http_smalltext = "GET /abc HTTP/1.1";
