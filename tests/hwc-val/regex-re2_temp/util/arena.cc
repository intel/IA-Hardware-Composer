...
void UnsafeArena::Reset() {
  ...
  const int overage =
      reinterpret_cast<uintptr_t>(freestart_) & (kDefaultAlignment - 1);
  if (overage > 0) {
...
UnsafeArena::AllocatedBlock* UnsafeArena::AllocNewBlock(
  ...
  if (blocks_alloced_ < arraysize(first_blocks_)) {
    ...
    if (overflow_blocks_ == NULL)
      overflow_blocks_ = new vector<AllocatedBlock>;
...
void* UnsafeArena::GetMemoryFallback(const size_t size, const int align) {
  ...
  const int overage = (reinterpret_cast<uintptr_t>(freestart_) & (align - 1));
  if (overage) {
