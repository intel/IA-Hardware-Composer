...
const HwcRect<int>& HwcLayer::GetLayerDamage() {
  HwcRect<int> transformed_damage = {0, 0, 0, 0};
  ...
  if (!surface_damage_.empty() &&
    ...
    transformed_damage = surface_damage_;
  }

  if (state_ & kVisibleRegionSet) {
    current_rendering_damage_ = Intersection(transformed_damage, visible_rect_);
  } else {
    current_rendering_damage_ = transformed_damage;
  }
